...
import app.argusos.extension.shared.settings.preference.SharedPrefCategory;
...
public abstract class Setting<T> {
     ...
     */
    public static final SharedPrefCategory preferences = new SharedPrefCategory("argusos_prefs");
     ...
     */
    private static final String OPTIONAL_ARGUSOS_SETTINGS_PREFIX = "argusos_";

    /**
     * The path, minus any 'argusos' prefix to keep json concise.
     */
    private String getImportExportKey() {
        if (key.startsWith(OPTIONAL_ARGUSOS_SETTINGS_PREFIX)) {
            return key.substring(OPTIONAL_ARGUSOS_SETTINGS_PREFIX.length());
        }
        return key;
    }
    ...
    public static boolean importFromJSON(Context alertDialogContext, String settingsJsonString) {
        try {
            ...
            Utils.runOnMainThreadDelayed(() -> Utils.showToastLong(numberOfSettingsImportedFinal == 0
                            ? str("argusos_settings_import_reset")
                            : str("argusos_settings_import_success", numberOfSettingsImportedFinal)),
                    150);

            return rebootSettingChanged;
        } catch (JSONException | IllegalArgumentException ex) {
            Utils.showToastLong(str("argusos_settings_import_failure_parse", ex.getMessage()));
            Logger.printInfo(() -> "", ex);
        } catch (Exception ex) {
