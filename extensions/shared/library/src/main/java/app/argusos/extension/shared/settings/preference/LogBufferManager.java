...
public final class LogBufferManager {
    ...
    public static void exportToClipboard() {
        try {
            ...
            if (logBuffer.isEmpty()) {
                Utils.showToastShort(str("argusos_debug_logs_none_found"));
            ...
            // Show a toast even if using Android 13+, but show ArgusOS toast first (before copying to clipboard).
            Utils.showToastShort(str("argusos_debug_logs_copied_to_clipboard"));

            Utils.setClipboard(String.join("\n", logBuffer));
        } catch (Exception ex) {
            // Handle security exception if clipboard access is denied.
            String errorMessage = String.format(str("argusos_debug_logs_failed_to_export"), ex.getMessage());
    ...
    public static void clearLogBuffer() {
        ...
        Utils.showToastShort(str("argusos_debug_logs_clear_toast"));
        clearLogBufferData();
    }
}
