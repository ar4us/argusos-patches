package app.argusos.extension.twitter.patches.hook.twifucker

import android.util.Log
import app.argusos.extension.twitter.patches.hook.twifucker.TwiFuckerUtils.forEach
import app.argusos.extension.twitter.patches.hook.twifucker.TwiFuckerUtils.forEachIndexed
...
internal object TwiFucker {
    ...
    private fun JSONObject.jsonCheckAndRemoveRecommendedUsers() {
        if (jsonHasRecommendedUsers()) {
            Log.d("ArgusOS", "Handle recommended users: $this")
    ...
    private fun JSONObject.jsonCheckAndRemoveThreads() {
        if (jsonHasThreads()) {
            Log.d("ArgusOS", "Handle threads: $this")
            jsonRemoveThreads()
        }
    }
    ...
    private fun JSONArray.trendRemoveAds() {
        ...
        forEachIndexed { trendIndex, trend ->
            if (trend.trendHasPromotedMetadata()) {
                Log.d("ArgusOS", "Handle trends ads $trendIndex $trend")
    ...
    private fun JSONArray.entriesRemoveTimelineAds() {
        ...
        forEachIndexed { entryIndex, entry ->
            ...
            if (entry.entryHasPromotedMetadata()) {
                Log.d("ArgusOS", "Handle timeline ads $entryIndex $entry")
            ...
            contentItems?.forEachIndexed inner@{ itemIndex, item ->
                if (item.entryHasPromotedMetadata()) {
                    Log.d("ArgusOS", "Handle timeline replies ads $entryIndex $entry")
    ...
    private fun JSONArray.entriesRemoveTweetDetailRelatedTweets() {
        ...
        forEachIndexed { entryIndex, entry ->
            ...
            if (entry.entryIsTweetDetailRelatedTweets()) {
                Log.d("ArgusOS", "Handle tweet detail related tweets $entryIndex $entry")
                removeIndex.add(entryIndex)
            }
        }
        for (i in removeIndex.reversed()) {
    ...
    fun JSONArray.entriesRemoveWhoToFollow() {
        ...
        forEachIndexed { entryIndex, entry ->
            ...
            Log.d("ArgusOS", "Handle whoToFollow $entryIndex $entry")
            ...
            items?.forEachIndexed { index, item ->
                item.itemContainsPromotedUser().let {
                    if (it) {
                        Log.d("ArgusOS", "Handle whoToFollow promoted user $index $item")
