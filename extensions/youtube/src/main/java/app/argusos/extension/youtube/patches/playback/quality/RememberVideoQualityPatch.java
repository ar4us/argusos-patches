package app.argusos.extension.youtube.patches.playback.quality;

import static app.argusos.extension.shared.StringRef.str;
import static app.argusos.extension.shared.Utils.NetworkType;
...
public class RememberVideoQualityPatch {
    ...
    public static void saveDefaultQuality(int qualityResolution) {
        ...
        if (Utils.getNetworkType() == NetworkType.MOBILE) {
            networkTypeMessage = str("argusos_remember_video_quality_mobile");
            qualitySetting = shortPlayerOpen ? shortsQualityMobile : videoQualityMobile;
        } else {
            networkTypeMessage = str("argusos_remember_video_quality_wifi");
        ...
        if (Settings.REMEMBER_VIDEO_QUALITY_LAST_SELECTED_TOAST.get()) {
            ...
            Utils.showToastShort(str(
                    shortPlayerOpen
                            ? "argusos_remember_video_quality_toast_shorts"
                            : "argusos_remember_video_quality_toast",
                    networkTypeMessage,
                    qualityLabel)
            );
        }
