package app.argusos.extension.youtube.returnyoutubedislike.requests;

import static app.argusos.extension.shared.StringRef.str;
import static app.argusos.extension.youtube.returnyoutubedislike.requests.ReturnYouTubeDislikeRoutes.getRYDConnectionFromRoute;
...
import app.argusos.extension.youtube.returnyoutubedislike.ReturnYouTubeDislike;
...
public class ReturnYouTubeDislikeAPI {
    ...
    private static void updateRateLimitAndStats(long timeNetworkCallStarted, boolean connectionError, boolean rateLimitHit) {
        ...
        if (connectionError) {
            ...
            if (!lastApiCallFailed && Settings.RYD_TOAST_ON_CONNECTION_ERROR.get()) {
                Utils.showToastLong(str("argusos_ryd_failure_client_rate_limit_requested"));
            }
            lastApiCallFailed = true;
        } else {
            fetchCallResponseTimeLast = responseTimeOfFetchCall;
    ...
    public static RYDVoteData fetchVotes(String videoId) {
        ...
        try {
            ...
            if (responseCode == HTTP_STATUS_CODE_SUCCESS) {
                ...
                handleConnectionError(str("argusos_ryd_failure_connection_status_code", responseCode),
                        responseCode, null, Toast.LENGTH_LONG);
            }
            connection.disconnect(); // Something went wrong, might as well disconnect.
        } catch (SocketTimeoutException ex) {
            handleConnectionError((str("argusos_ryd_failure_connection_timeout")), null, ex, Toast.LENGTH_SHORT);
        } catch (IOException ex) {
            handleConnectionError((str("argusos_ryd_failure_generic", ex.getMessage())), null, ex, Toast.LENGTH_LONG);
        } catch (Exception ex) {
            // should never happen
    ...
    public static String registerAsNewUser() {
        ...
        try {
            ...
            handleConnectionError(str("argusos_ryd_failure_generic", "registration failed"), null, ex, Toast.LENGTH_LONG);
        } catch (Exception ex) {
    ...
    private static String confirmRegistration(String userID, String solution) {
        ...
        try {
            ...
            handleConnectionError(str("argusos_ryd_failure_generic", "confirm registration failed"),
    ...
    public static boolean sendVote(String videoId, ReturnYouTubeDislike.Vote vote) {
        ...
        try {
            ...
            handleConnectionError(str("argusos_ryd_failure_generic", "send vote failed"), null, ex, Toast.LENGTH_LONG);
        } catch (Exception ex) {
            // should never happen
    ...
    private static boolean confirmVote(String videoId, String userID, String solution) {
        ...
        try {
            ...
            handleConnectionError(str("argusos_ryd_failure_generic", "confirm vote failed"),
                    null, ex, Toast.LENGTH_LONG);
        } catch (Exception ex) {
