package app.argusos.extension.youtube.returnyoutubedislike.ui;
...
import app.argusos.extension.youtube.returnyoutubedislike.requests.ReturnYouTubeDislikeAPI;
...
public class ReturnYouTubeDislikeDebugStatsPreferenceCategory extends PreferenceCategory {
    ...
    private static String createMillisecondStringFromNumber(long number) {
        return String.format(str("argusos_ryd_statistics_millisecond_text"), number);
    }
    ...
    protected void onAttachedToActivity() {
        try {
            ...
            if (fetchCallTimeWaitingLast == ReturnYouTubeDislikeAPI.FETCH_CALL_RESPONSE_TIME_VALUE_RATE_LIMIT) {
                fetchCallTimeWaitingLastSummary = str("argusos_ryd_statistics_getFetchCallResponseTimeLast_rate_limit_summary");
            } else {
                fetchCallTimeWaitingLastSummary = createMillisecondStringFromNumber(fetchCallTimeWaitingLast);
            }
            addStatisticPreference(
                    "argusos_ryd_statistics_getFetchCallResponseTimeLast_title",
                    fetchCallTimeWaitingLastSummary
            );

            addStatisticPreference(
                    "argusos_ryd_statistics_getFetchCallCount_title",
                    createSummaryText(ReturnYouTubeDislikeAPI.getFetchCallCount(),
                            "argusos_ryd_statistics_getFetchCallCount_zero_summary",
                            "argusos_ryd_statistics_getFetchCallCount_non_zero_summary"
                    )
            );

            addStatisticPreference(
                    "argusos_ryd_statistics_getFetchCallNumberOfFailures_title",
                    createSummaryText(ReturnYouTubeDislikeAPI.getFetchCallNumberOfFailures(),
                            "argusos_ryd_statistics_getFetchCallNumberOfFailures_zero_summary",
                            "argusos_ryd_statistics_getFetchCallNumberOfFailures_non_zero_summary"
                    )
            );

            addStatisticPreference(
                    "argusos_ryd_statistics_getNumberOfRateLimitRequestsEncountered_title",
                    createSummaryText(ReturnYouTubeDislikeAPI.getNumberOfRateLimitRequestsEncountered(),
                            "argusos_ryd_statistics_getNumberOfRateLimitRequestsEncountered_zero_summary",
                            "argusos_ryd_statistics_getNumberOfRateLimitRequestsEncountered_non_zero_summary"
                    )
            );
        } catch (Exception ex) {
