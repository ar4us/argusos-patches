package app.argusos.extension.youtube.settings.preference;
...
public class SpoofVideoStreamsSideEffectsPreference extends Preference {
    ...
    private final SharedPreferences.OnSharedPreferenceChangeListener listener = (sharedPreferences, str) -> {
        // Because this listener may run before the ArgusOS settings fragment updates Settings,
    ...
    private void updateUI() {
        ...
        switch (clientType) {
            case ANDROID_CREATOR ->
                    summary = str("argusos_spoof_video_streams_about_no_audio_tracks")
                            + '\n' + str("argusos_spoof_video_streams_about_no_stable_volume")
                            + '\n' + str("argusos_spoof_video_streams_about_no_av1")
                            + '\n' + str("argusos_spoof_video_streams_about_no_force_original_audio");
            case ANDROID_REEL ->
                    summary = str("argusos_spoof_video_streams_about_playback_failure");
            // VR 1.61 is not exposed in the UI and should never be reached here.
            case ANDROID_VR_1_43_32, ANDROID_VR_1_61_48 ->
                    summary = str("argusos_spoof_video_streams_about_playback_failure")
                            + '\n' + str("argusos_spoof_video_streams_about_no_audio_tracks")
                            + '\n' + str("argusos_spoof_video_streams_about_no_stable_volume");
            case VISIONOS -> summary = str("argusos_spoof_video_streams_about_experimental")
                    + '\n' + str("argusos_spoof_video_streams_about_playback_failure")
                    + '\n' + str("argusos_spoof_video_streams_about_no_audio_tracks")
                    + '\n' + str("argusos_spoof_video_streams_about_no_av1");
        ...
        if (!clientType.name().startsWith("ANDROID_VR") && clientType != ClientType.ANDROID_REEL) {
            summary += '\n' + str("argusos_spoof_video_streams_about_no_immersive_mode");
        }
