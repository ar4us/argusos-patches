package app.argusos.extension.youtube.sponsorblock;
...
import app.argusos.extension.youtube.patches.VideoInformation;
import app.argusos.extension.youtube.settings.Settings;
import app.argusos.extension.youtube.sponsorblock.objects.CategoryBehaviour;
import app.argusos.extension.youtube.sponsorblock.objects.SegmentCategory;
import app.argusos.extension.youtube.sponsorblock.objects.SponsorSegment;
import app.argusos.extension.youtube.sponsorblock.objects.SponsorSegment.SegmentVote;
import app.argusos.extension.youtube.sponsorblock.requests.SBRequester;
import app.argusos.extension.youtube.sponsorblock.ui.SponsorBlockViewController;
...
public class SponsorBlockUtils {
        ...
        public void onClick(DialogInterface dialog, int which) {
            try {
                ...
                if (category.behaviour == CategoryBehaviour.IGNORE) {
                    Utils.showToastLong(str("argusos_sb_new_segment_disabled_category"));
                    enableButton = false;
                } else {
        ...
        public void onClick(DialogInterface dialog, int which) {
            try {
                ...
                new AlertDialog.Builder(context)
                        .setTitle(str("argusos_sb_new_segment_choose_category"))
    ...
    private static final DialogInterface.OnClickListener editByHandDialogListener = (dialog, which) -> {
        try {
            ...
            new AlertDialog.Builder(context)
                    .setTitle(str(isStart ? "argusos_sb_new_segment_time_start" : "argusos_sb_new_segment_time_end"))
                    .setView(textView)
                    .setNegativeButton(android.R.string.cancel, null)
                    .setNeutralButton(str("argusos_sb_new_segment_now"), editByHandSaveDialogListener)
    ...
    public static void onMarkLocationClicked() {
        try {
            ...
            new AlertDialog.Builder(SponsorBlockViewController.getOverLaysViewGroupContext())
                    .setTitle(str("argusos_sb_new_segment_title"))
                    .setMessage(str("argusos_sb_new_segment_mark_time_as_question",
                            formatSegmentTime(newSponsorSegmentDialogShownMillis)))
                    .setNeutralButton(android.R.string.cancel, null)
                    .setNegativeButton(str("argusos_sb_new_segment_mark_start"), newSponsorSegmentDialogListener)
                    .setPositiveButton(str("argusos_sb_new_segment_mark_end"), newSponsorSegmentDialogListener)
                    .show();
        } catch (Exception ex) {
    ...
    public static void onPublishClicked() {
        try {
            ...
            if (newSponsorSegmentStartMillis < 0 || newSponsorSegmentEndMillis < 0) {
                Utils.showToastShort(str("argusos_sb_new_segment_mark_locations_first"));
            } else if (newSponsorSegmentStartMillis >= newSponsorSegmentEndMillis) {
                Utils.showToastShort(str("argusos_sb_new_segment_start_is_before_end"));
            } else if (!newSponsorSegmentPreviewed && newSponsorSegmentStartMillis != 0) {
                Utils.showToastLong(str("argusos_sb_new_segment_preview_segment_first"));
            } else {
                final long segmentLength = (newSponsorSegmentEndMillis - newSponsorSegmentStartMillis) / 1000;
                new AlertDialog.Builder(SponsorBlockViewController.getOverLaysViewGroupContext())
                        .setTitle(str("argusos_sb_new_segment_confirm_title"))
                        .setMessage(str("argusos_sb_new_segment_confirm_content",
    ...
    public static void onVotingClicked(Context context) {
        try {
            ...
            if (segments == null || segments.length == 0) {
                ...
                Utils.showToastShort(str("argusos_sb_vote_no_segments"));
                return;
            }
            ...
            for (int i = 0; i < numberOfSegments; i++) {
                ...
                if (segment.category == SegmentCategory.HIGHLIGHT) {
                    ...
                    String toFromString = str("argusos_sb_vote_segment_time_to_from",
    ...
    public static void onEditByHandClicked() {
        try {
            Utils.verifyOnMainThread();
            new AlertDialog.Builder(SponsorBlockViewController.getOverLaysViewGroupContext())
                    .setTitle(str("argusos_sb_new_segment_edit_by_hand_title"))
                    .setMessage(str("argusos_sb_new_segment_edit_by_hand_content"))
                    .setNeutralButton(android.R.string.cancel, null)
                    .setNegativeButton(str("argusos_sb_new_segment_mark_start"), editByHandDialogListener)
                    .setPositiveButton(str("argusos_sb_new_segment_mark_end"), editByHandDialogListener)
                    .show();
        } catch (Exception ex) {
    ...
    public static String getTimeSavedString(long totalSecondsSaved) {
        ...
        if (hours > 0) {
            ...
            return str("argusos_sb_stats_saved_hour_format", hoursFormatted, minutesFormatted);
        }

        final long seconds = duration.getSeconds() % 60;
        String secondsFormatted = statsNumberFormatter.format(seconds);
        if (minutes > 0) {
            return str("argusos_sb_stats_saved_minute_format", minutesFormatted, secondsFormatted);
        }

        return str("argusos_sb_stats_saved_second_format", secondsFormatted);
    }
        ...
        public void onClick(DialogInterface dialog, int which) {
            try {
                ...
                if (which == DialogInterface.BUTTON_NEUTRAL) {
                    ...
                    if (time < 0) {
                        Utils.showToastLong(str("argusos_sb_new_segment_edit_by_hand_parse_error"));
                        return;
                    }
                }
