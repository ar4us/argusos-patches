...
public enum CategoryBehaviour {
    SKIP_AUTOMATICALLY("skip", 2, true, sf("argusos_sb_skip_automatically")),
    // desktop does not have skip-once behavior. Key is unique to ArgusOS
    SKIP_AUTOMATICALLY_ONCE("skip-once", 3, true, sf("argusos_sb_skip_automatically_once")),
    MANUAL_SKIP("manual-skip", 1, false, sf("argusos_sb_skip_showbutton")),
    SHOW_IN_SEEKBAR("seekbar-only", 0, false, sf("argusos_sb_skip_seekbaronly")),
    // ignored categories are not exported to JSON, and ignore is the default behavior when importing
    IGNORE("ignore", -1, false, sf("argusos_sb_skip_ignore"));

    /**
     * ArgusOS specific value.
     */
    @NonNull
    ...
    @Nullable
    public static CategoryBehaviour byArgusOSKeyValue(@NonNull String keyValue) {
