package app.argusos.extension.youtube.sponsorblock.requests;
...
public class SBRequester {
    ...
    public static SponsorSegment[] getSegments(String videoId) {
        ...
        try {
            ...
            if (responseCode == HTTP_STATUS_CODE_SUCCESS) {
                ...
                handleConnectionError(str("argusos_sb_sponsorblock_connection_failure_status", responseCode), null);
                connection.disconnect(); // something went wrong, might as well disconnect
            }
        } catch (SocketTimeoutException ex) {
            handleConnectionError(str("argusos_sb_sponsorblock_connection_failure_timeout"), ex);
        } catch (IOException ex) {
            handleConnectionError(str("argusos_sb_sponsorblock_connection_failure_generic"), ex);
        } catch (Exception ex) {
            // Should never happen
    ...
    public static void submitSegments(String videoId, String category,
        ...
        try {
            ...
            if (responseCode == HTTP_STATUS_CODE_SUCCESS) {
                Utils.showToastLong(str("argusos_sb_submit_succeeded"));
                return;
            }

            String userErrorMessage = switch (responseCode) {
                case 409 -> str("argusos_sb_submit_failed_duplicate");
                case 403 -> str("argusos_sb_submit_failed_forbidden",
                        Requester.parseErrorStringAndDisconnect(connection));
                case 429 -> str("argusos_sb_submit_failed_rate_limit");
                case 400 -> str("argusos_sb_submit_failed_invalid",
                        Requester.parseErrorStringAndDisconnect(connection));
                default -> str("argusos_sb_submit_failed_unknown_error",
            ...
            Utils.showToastLong(str("argusos_sb_submit_failed_timeout"));
        } catch (IOException ex) {
            Logger.printDebug(() -> "IOException", ex);
            Utils.showToastLong(str("argusos_sb_submit_failed_unknown_error", 0, ex.getMessage()));
        } catch (Exception ex) {
            ...
            try {
                ...
                switch (responseCode) {
                    case HTTP_STATUS_CODE_SUCCESS:
                        ...
                        return;
                    case 403:
                        userMessage = str("argusos_sb_vote_failed_forbidden",
                                Requester.parseErrorStringAndDisconnect(connection));
                        break;
                    default:
                        userMessage = str("argusos_sb_vote_failed_unknown_error",
                                responseCode, connection.getResponseMessage());
                        break;
                }
                ...
                Utils.showToastShort(str("argusos_sb_vote_failed_timeout"));
            } catch (IOException ex) {
                Utils.showToastShort(str("argusos_sb_vote_failed_unknown_error", 0, ex.getMessage()));
            } catch (Exception ex) {
    ...
    public static String setUsername(String username) {
        ...
        try {
            ...
            return str("argusos_sb_stats_username_change_unknown_error", responseCode, responseMessage);
        } catch (Exception ex) { // should never happen
            Logger.printInfo(() -> "failed to set username", ex); // do not toast
            return str("argusos_sb_stats_username_change_unknown_error", 0, ex.getMessage());
        }
    }
