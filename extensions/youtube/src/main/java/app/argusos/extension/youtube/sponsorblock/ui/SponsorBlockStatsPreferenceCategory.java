...
import app.argusos.extension.youtube.sponsorblock.SponsorBlockUtils;
import app.argusos.extension.youtube.sponsorblock.objects.UserStats;
...
public class SponsorBlockStatsPreferenceCategory extends PreferenceCategory {
    ...
    protected void onAttachedToActivity() {
        try {
            ...
            if (enabled) {
                loadingPlaceholderPreference.setTitle(str("argusos_sb_stats_loading"));
                ...
                loadingPlaceholderPreference.setTitle(str("argusos_sb_stats_sb_disabled"));
            }
        } catch (Exception ex) {
    ...
    private void addUserStats(Preference loadingPlaceholder, @Nullable UserStats stats) {
        ...
        try {
            if (stats == null) {
                loadingPlaceholder.setTitle(str("argusos_sb_stats_connection_failure"));
                return;
            }
            removeAll();
            ...
            if (stats.totalSegmentCountIncludingIgnored > 0) {
                ...
                preference.setTitle(fromHtml(str("argusos_sb_stats_username", userName), FROM_HTML_MODE_COMPACT));
                preference.setSummary(str("argusos_sb_stats_username_change"));
                    ...
                    Utils.runOnBackgroundThread(() -> {
                        ...
                        Utils.runOnMainThread(() -> {
                            if (errorMessage == null) {
                                preference.setTitle(fromHtml(str("argusos_sb_stats_username", newUserName), FROM_HTML_MODE_COMPACT));
                                preference.setText(newUserName);
                                Utils.showToastLong(str("argusos_sb_stats_username_changed"));
                            } else {
...

            {
                ...
                preference.setTitle(fromHtml(str("argusos_sb_stats_submissions", formatted), FROM_HTML_MODE_COMPACT));
                preference.setSummary(str("argusos_sb_stats_submissions_sum"));
                if (stats.totalSegmentCountIncludingIgnored == 0) {
...

            {
                ...
                Preference preference = new Preference(context);
                preference.setTitle(fromHtml(str("argusos_sb_stats_reputation", stats.reputation), FROM_HTML_MODE_COMPACT));
                preference.setSelectable(false);
                preference.setEnabled(Settings.SB_ENABLED.isAvailable());
...

            {
                ...
                if (stats.totalSegmentCountIncludingIgnored == 0) {
                    stats_saved = str("argusos_sb_stats_saved_zero");
                    stats_saved_sum = str("argusos_sb_stats_saved_sum_zero");
                } else {
                    stats_saved = str("argusos_sb_stats_saved",
                            SponsorBlockUtils.getNumberOfSkipsString(stats.viewCount));
                    stats_saved_sum = str("argusos_sb_stats_saved_sum",
    ...
    private void addLocalUserStats() {
        ...
        Runnable updateStatsSelfSaved = () -> {
            ...
            preference.setTitle(fromHtml(str("argusos_sb_stats_self_saved", formatted), FROM_HTML_MODE_COMPACT));

            String formattedSaved = SponsorBlockUtils.getTimeSavedString(
                    Settings.SB_LOCAL_TIME_SAVED_MILLISECONDS.get() / 1000);
            preference.setSummary(fromHtml(str("argusos_sb_stats_self_saved_sum", formattedSaved), FROM_HTML_MODE_COMPACT));
        };
        ...
        preference.setOnPreferenceClickListener(preference1 -> {
            Pair<Dialog, LinearLayout> dialogPair = CustomDialog.create(
                    ...
                    str("argusos_sb_stats_self_saved_reset_title"), // Title.
                    null, // No message.
                    null, // No EditText.
                    null, // OK button text.
                    () -> {
