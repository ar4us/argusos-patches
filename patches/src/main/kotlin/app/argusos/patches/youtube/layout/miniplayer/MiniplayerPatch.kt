...
package app.argusos.patches.youtube.layout.miniplayer
...
private const val EXTENSION_CLASS_DESCRIPTOR =
    "Lapp/argusos/extension/youtube/patches/MiniplayerPatch;"
    ...
    apply {
        ...
        preferences +=
            if (is_20_37_or_greater) {
                ListPreference("argusos_miniplayer_type")
            } else if (is_20_03_or_greater) {
                ListPreference(
                    key = "argusos_miniplayer_type",
                    entriesKey = "argusos_miniplayer_type_legacy_20_03_entries",
                    entryValuesKey = "argusos_miniplayer_type_legacy_20_03_entry_values",
                )
            } else if (is_19_43_or_greater) {
                ListPreference(
                    key = "argusos_miniplayer_type",
                    entriesKey = "argusos_miniplayer_type_legacy_19_43_entries",
                    entryValuesKey = "argusos_miniplayer_type_legacy_19_43_entry_values",
                )
            } else {
                ListPreference(
                    key = "argusos_miniplayer_type",
                    entriesKey = "argusos_miniplayer_type_legacy_19_16_entries",
                    entryValuesKey = "argusos_miniplayer_type_legacy_19_16_entry_values",
                )
            }

        if (is_19_25_or_greater) {
            if (!is_19_29_or_greater) {
                preferences += SwitchPreference("argusos_miniplayer_double_tap_action")
            }
            preferences += SwitchPreference("argusos_miniplayer_disable_drag_and_drop")
        }

        if (is_19_43_or_greater) {
            preferences += SwitchPreference("argusos_miniplayer_disable_horizontal_drag")
        }

        if (is_19_36_or_greater) {
            preferences += SwitchPreference("argusos_miniplayer_disable_rounded_corners")
        }

        preferences += SwitchPreference("argusos_miniplayer_hide_subtext")

        preferences += if (is_19_26_or_greater) {
            SwitchPreference("argusos_miniplayer_hide_overlay_buttons")
        } else {
            SwitchPreference(
                key = "argusos_miniplayer_hide_overlay_buttons",
                titleKey = "argusos_miniplayer_hide_overlay_buttons_legacy_title",
                summaryOnKey = "argusos_miniplayer_hide_overlay_buttons_legacy_summary_on",
                summaryOffKey = "argusos_miniplayer_hide_overlay_buttons_legacy_summary_off",
            )
        }

        if (!is_19_26_or_greater) {
            preferences += SwitchPreference("argusos_miniplayer_hide_rewind_forward")
        }

        if (is_19_26_or_greater) {
            preferences += TextPreference(
                "argusos_miniplayer_width_dip",
                inputType = InputType.NUMBER
            )
        }

        preferences += TextPreference("argusos_miniplayer_opacity", inputType = InputType.NUMBER)

        PreferenceScreen.PLAYER.addPreferences(
            PreferenceScreenPreference(
                key = "argusos_miniplayer_screen",
